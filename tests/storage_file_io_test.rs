...
use common::{TestEnv, create_mock_harmony_instance};
...
fn test_save_harmony_instances_creates_file() {
    ...
    // Write instances
    let instances = serde_json::json!([create_mock_harmony_instance(
        "abc123",
        "127.0.0.1",
        8081,
        "test",
...
fn test_save_harmony_instances_overwrites_existing() {
    ...
    // Write initial data
    let initial = serde_json::json!([create_mock_harmony_instance(
        "old123",
        "127.0.0.1",
        8081,
        "old",
    ...
    let updated = serde_json::json!([create_mock_harmony_instance(
        "new456",
        "192.168.1.1",
        9000,
        "new",
...
fn test_deduplication_by_label_concept() {
    ...
    let production_count = array.iter().filter(|i| i["label"] == "production").count();

    assert_eq!(
        production_count, 1,
        "Should have exactly one 'production' instance"
    );
}
...
fn test_deduplication_by_address_concept() {
    ...
    assert_eq!(
        address_count, 1,
        "Should have exactly one instance at this address"
    );
}
...
fn test_id_generation_different_inputs() {
    ...
    assert_ne!(
        instance1["id"], instance2["id"],
        "Different ports should have different IDs"
    );
    assert_ne!(
